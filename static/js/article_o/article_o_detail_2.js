...
function comm_zan(view, id, ths, cls) {
    var url = "/article/comment_zan/";
    $.ajax({
        url: url,
        type: "post",
        headers: {"X-CSRFToken": $.cookie("csrftoken")},
        ...
        success: function (res) {
            if (res.code != '403'){
                if (cls == "comm_good") {
                    if (res.is_view == 1) {
                        $(ths).next().next().next().next().addClass('alert-warning').html('已经评价过').show().delay(1000).fadeOut();
                    }
                    else {
                        $(ths).next().text('(' + res.good_view.toString() + ')');
                        $(ths).next().next().next().text('(' + res.bad_view.toString() + ')');
                    ...
                    if (res.is_view == 1) {
                        $(ths).next().next().addClass('alert-warning').html('已经评价过').show().delay(1000).fadeOut();
                    }
                    else {
                        $(ths).next().text('(' + res.bad_view.toString() + ')');
                        $(ths).prev().text('(' + res.good_view.toString() + ')');
...
$(".del_com").click(function () {
   ...
   $.ajax({
       ...
       success: function () {
           ...
           $(this).siblings('p').find('span:first').addClass("bad").text("该评论已删除");
       }.bind(this)
   })
});
...
$(".comm_good").click(function () {
    var view = 1;
    var id = $(this).attr("id_val");
    var cls = "comm_good";
    comm_zan(view, id, this, cls);
});

$(".comm_bad").click(function () {
    var view = 2;
    var id = $(this).attr("id_val");
    var cls = "comm_bad";
    comm_zan(view, id, this, cls);
